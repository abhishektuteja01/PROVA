...
function ScoreCard() {
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-[15%] right-[15%] h-[1px]" style={{ background: "linear-gradient(90deg,transparent,rgba(59,130,246,0.35),transparent)" }} />
      ...
      {[
        ...
        return (
          <div key={short} style={{ marginBottom: "15px" }}>
            ...
            <div style={{ height: "3px", background: "rgba(255,255,255,0.03)", borderRadius: "0px", overflow: "hidden" }}>
      ...
      <div style={{ marginTop: "12px", paddingTop: "14px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between" }}>
        <span style={{ fontFamily: "var(--font-ibm-plex-mono)", fontSize: "10px", color: "rgba(156,163,175,0.28)", letterSpacing: "0.06em" }}>CREDIT RISK MODEL v3</span>
        <span style={{ fontFamily: "var(--font-ibm-plex-mono)", fontSize: "10px", color: "rgba(156,163,175,0.28)" }}>ACME BANK</span>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div style={{ background: "var(--color-bg-primary)", minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        <div style={{ position: "absolute", top: "-15%", right: "-8%", width: "600px", height: "600px", borderRadius: "50%", background: "radial-gradient(circle, rgba(59,130,246,0.06) 0%, transparent 65%)", filter: "blur(60px)", animation: "orb1 24s ease-in-out infinite" }} />
        <div style={{ position: "absolute", bottom: "-18%", left: "-8%", width: "500px", height: "500px", borderRadius: "50%", background: "radial-gradient(circle, rgba(59,130,246,0.04) 0%, transparent 65%)", filter: "blur(72px)", animation: "orb2 30s ease-in-out infinite" }} />
      ...
      <section className="relative z-10 min-h-screen flex flex-col md:flex-row items-center px-5 py-24 md:px-20 md:py-28 max-w-[1280px] mx-auto gap-12 md:gap-20">
        <div className="flex-1 min-w-0">
          <motion.div
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: "8px", marginBottom: "32px", padding: "5px 14px", border: "1px solid rgba(59,130,246,0.2)", borderRadius: "100px", background: "rgba(59,130,246,0.06)" }}
          >
          ...
          <motion.div
            ...
            className="flex flex-wrap md:flex-nowrap pt-9 border-t border-white/5"
          >
            {[
              ...
              <div key={label} className={`flex-1 min-w-[120px] mb-6 md:mb-0 ${i > 0 ? "md:pl-7 md:ml-7 md:border-l border-white/5" : ""}`}>
          ...
          {PILLARS.map((pillar, i) => (
            <FadeIn key={pillar.code} delay={i * 0.08} style={{ flex: 1 }}>
              <div style={{ height: "100%", background: "var(--color-bg-secondary)", border: "1px solid var(--color-border)", borderRadius: "2px", padding: "28px", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", top: 0, left: "20%", right: "20%", height: "1px", background: "linear-gradient(90deg,transparent,rgba(59,130,246,0.16),transparent)" }} />
                ...
                <div style={{ marginBottom: "18px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                    <span style={{ fontFamily: "var(--font-geist)", fontSize: "11px", color: "rgba(156,163,175,0.45)" }}>Sample score</span>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                  {pillar.elements.map(el => (
                    <span key={el} style={{ fontFamily: "var(--font-ibm-plex-mono)", fontSize: "10px", color: "rgba(156,163,175,0.38)", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", padding: "2px 7px", borderRadius: "3px" }}>
                      {el}
                    </span>
                  ))}
                </div>
      ...
      <section className="relative z-10 max-w-[1280px] mx-auto px-5 md:px-20 py-5 md:py-24">
        ...
        <div className="flex flex-col md:flex-row relative">
          <div style={{ position: "absolute", top: "19px", left: "20px", right: "20px", height: "1px", background: "rgba(255,255,255,0.07)" }}>
            <motion.div
              initial={{ width: "0%" }}
              whileInView={{ width: "100%" }}
              viewport={{ once: true, margin: "-40px" }}
              transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}
              style={{ height: "100%", background: "linear-gradient(90deg, transparent, rgba(59,130,246,0.35), transparent)" }}
            />
          </div>
      ...
      <section className="relative z-10 max-w-[1280px] mx-auto px-5 md:px-20 mb-20">
        <FadeIn>
          <div className="bg-accent-[0.04] border border-accent/10 p-9 md:p-13 flex flex-col md:flex-row items-center justify-between gap-10" style={{ borderRadius: "2px" }}>
            ...
            <Link
              href="/signup"
              className="bg-accent text-bg-primary font-sans text-[14px] font-medium px-7 py-3 w-full md:w-auto text-center shrink-0 transition-opacity hover:opacity-85"
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t border-white/5 px-5 md:px-20 py-7 max-w-[1280px] mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        <span className="font-serif text-[17px] text-text-secondary">Prova</span>
        <span className="font-sans text-[12px] text-white/10 text-center">
        ...
        <div className="flex gap-6">
          {([["Sign in", "/login"], ["Get started", "/signup"]] as const).map(([label, href]) => (
            <Link
              key={href}
              href={href}
              className="font-sans text-[13px] text-white/20 transition-colors hover:text-text-secondary"
